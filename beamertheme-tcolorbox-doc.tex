...
There is also an experimental \saminline|xbeamer| version of the tcolorbox inner theme available from its github page\footnote{\url{https://github.com/samcarter/beamertheme-tcolorbox}}.

Most of the theme options are the same, only the \saminline|inmargin| option does not exist:
\begin{tcblisting}{
  title={xbeamer theme},
  listing and comment,
  pdf comment,
  freeze pdf,
  compilable listing,
  run latexmk={-lualatex -pdflualatex=lualatex-dev  -cd},
...
Internally, the tcolorbox xbeamer theme works a bit differently than the normal beamer theme and thus making customisations is a bit different, too. The xbeamer theme uses a set of tcolorbox styles to store the options used for the different blocks and boxes:
\begin{center}
\begin{forest}
  baseline,
  for tree={%
...
\begin{tcblisting}{
  ...
  \usepackage{xbeamerthemetcolorbox}
